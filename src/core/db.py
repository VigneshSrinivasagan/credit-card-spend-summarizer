...
#
# Queries multimodal_chunks DIRECTLY using raw SQL + pgvector <=> operator.
#
# WHY not LangChain PGVector wrapper:
#   Ingestion wrote rows directly into multimodal_chunks via psycopg raw SQL.
#   LangChain's PGVector wrapper manages its OWN tables (langchain_pg_collection
#   and langchain_pg_embedding) — it has no awareness of multimodal_chunks.
#   Using the wrapper would return zero results every time.
#
#   Instead we embed the query ourselves (same model as ingestion) and run
...
def similarity_search(query: str, k: int = 20) -> list[Document]:
    ...
    Returns LangChain Document objects so they plug directly into
    rerank_node and generate_answer_node without any changes.
    """
...
def get_sql_database() -> SQLDatabase:
    """Return a LangChain SQLDatabase connected to the credit_card_summary_seed_data_db (read-only).

    Uses the rag_readonly role from sql/seed.sql — SELECT privileges only.
    Connection string is read from PG_CONNECTION_STRING_RDBMS in the environment.
    """
    db_url = os.getenv("PG_CONNECTION_STRING_RDBMS")
    if not db_url:
